...
void mixColumns(unsigned char stat[N][N]) {
  ...
  // printf("mixColumns step\n");
   ...
   for (int colone = 0; colone < N; colone++) {
      ...
      multiplicationEtReafectaion(stat, colone, s);
   }
...
void multiplicationEtReafectaion(unsigned char stat[N][N], int column, unsigned char s[N]) {
   ...
   for (int i = 0; i < N; i++) {
      ...
      tmpS[i] = EvaluationPremierExpressiondeS(tmp) ^ s[(i + 1) % N] ^ s[(i + 2) % N] ^ s[(i + 3) % N];
